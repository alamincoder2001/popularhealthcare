				...
				<table class="record-table" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'with_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'with_details' ? '' : 'none'}">
					...
					<tbody>
						...
						<tr style="font-weight:bold;">
							...
							<td style="text-align:center;">
								{{ sales.reduce((allTotal, product) => {
									sum = product.saleDetails.reduce((total, record) => {
										return total + parseFloat(record.SaleDetails_TotalQuantity);
									}, 0);
									return parseFloat(parseFloat(allTotal) + parseFloat(sum));
									}, 0)
								}}
							</td>
							<td></td>
						...
						<tbody>
							<template v-for="sale in sales">
								...
								<tr style="font-weight:bold;">
									...
									<td style="text-align:right;">{{ sale.products.reduce((prev, curr) => { return prev + parseFloat(curr.quantity)}, 0) }}</td>
								</tr>
							</template>
						</tbody>
					</table>
...
<script>
	...
	new Vue({
		...
		methods: {
			filterCustomer() {
				let sl = this.selectedEmployee.Employee_SlNo;
				axios.get('/get_customers').then(res => {
					this.customers = res.data.filter(item => {
						return item.Derma_Id == sl || item.Nutrition_Id == sl || item.Healthcare_Id == sl
					});
				})
			},
				...
				axios.get('/check_sale_return/' + sale.SaleMaster_InvoiceNo).then(res => {
					if (res.data.found) {
						...
						if (sale.is_service == 'true') {
							window.open('/sales/service/' + sale.SaleMaster_SlNo, '_blank');
						} else {
							window.open('/sales/product/' + sale.SaleMaster_SlNo, '_blank');
						}
					}
				})
			},
