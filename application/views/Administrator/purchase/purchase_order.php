				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					<thead>
						<tr>
							...
							<th style="width:10%;color:#000;">Manuf. Date</th>
							<th style="width:10%;color:#000;">Ex. Date</th>
							<th style="width:10%;color:#000;">Batch No.</th>
							<th style="width:10%;color:#000;">Purchase Rate</th>
							<th style="width:8%;color:#000;">Quantity</th>
							<th style="width:10%;color:#000;">Total Amount</th>
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td><input style="width: 60px; text-align:center;" type="number" v-model="product.quantity" v-on:input="calculateProductQty(sl)"></td>
							<td>{{ product.total }}</td>
	...
	new Vue({
		...
		methods:{
			...
			calculateProductQty(ind) {
				this.cart[ind].total = this.cart[ind].quantity * this.cart[ind].purchaseRate;
				this.calculateTotal()
			},
			addToCart(){
