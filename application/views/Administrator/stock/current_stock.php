...
<div id="stock">
	<div class="row">
		<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
			...
			<div class="form-group" style="margin-top:10px;" v-if="selectedSearchType.value == 'product' || selectedSearchType.value == 'invoice'">
				<div class="col-sm-2" style="margin-left:15px;">
		...
		<div class="col-md-12">
			<div class="table-responsive" id="stockContent">
				<table class="table table-bordered" v-if="searchType == 'current'" style="display:none" v-bind:style="{display: searchType == 'current' ? '' : 'none'}">
					...
					<tfoot>
						<tr>
							...
							<th style="text-align:right;">Total Sale Value</th>
							<th>{{ saleTotalValue | decimal }}</th>
						</tr>
					</tfoot>
				</table>

				<table class="table table-bordered" style="display: none;" v-bind:style="{display: searchType == 'invoice' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No</th>
							<th>Product Name</th>
					...
					<tbody>
						<tr v-for="product in stock">
							...
							<td style="text-align:right;">{{ product.PurchaseDetails_Rate * product.PurchaseDetails_TotalQuantity }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr>
							<th colspan="4" style="text-align:right;">Total:</th>
							<th>{{ parseFloat(stock.reduce((prev, curr) => {return prev + parseFloat(curr.PurchaseDetails_TotalQuantity)}, 0)).toFixed(2) }}</th>
							<th style="text-align:right;">{{ parseFloat(stock.reduce((prev, curr) => {return prev + parseFloat(curr.PurchaseDetails_Rate * curr.PurchaseDetails_TotalQuantity)}, 0)).toFixed(2) }}</th>
						</tr>
					</tfoot>
				</table>
				...
				<table class="table table-bordered" 
						...
						style="display:none;" 
						v-bind:style="{display: searchType != 'current' && searchType != null ? (searchType != 'invoice' ? '' : 'none') : 'none'}">
					<thead>
						<tr>
							<th>Product Id</th>
							<th>Product Name</th>
	...
	new Vue({
		...
		data(){
			return {
				searchTypes: [
					...
					{text: 'Invoice Wise Stock', value: 'invoice'},
		...
		methods:{
			getStock(){
				...
				if(this.searchType == 'invoice' && this.selectedProduct == null){
					alert('Select a product');
					return;
				} else if(this.searchType == 'invoice' && this.selectedProduct != null) {
					parameters.productId = this.selectedProduct.Product_SlNo;
					this.selectionText = "product: " + this.selectedProduct.display_text;
				} 
				...
				axios.post(url, parameters).then(res => {
					...
					this.saleTotalValue = res.data.saleTotalValue;
				})
			},
			onChangeSearchType(){
				if(this.selectedSearchType.value == 'category' && this.categories.length == 0){
					...
					this.getProducts();
				} else if(this.selectedSearchType.value == 'invoice' && this.products.length == 0){
					this.getProducts();
				}
			},
			getCategories(){
