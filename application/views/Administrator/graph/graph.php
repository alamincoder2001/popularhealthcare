...
<div id="graph">
    <div class="row" v-if="showData" style="display:none;" v-bind:style="{ display: showData ? '' : 'none' }">
        <div class="col-md-12">
            <marquee scrollamount="3" onmouseover="this.stop();" onmouseout="this.start();" direction="left" height="30" bgcolor="#224079" style="color:white;padding-top:5px;">{{ salesText }}</marquee>
        </div>
    </div>
    <?php if ($this->session->userdata('BRANCHid') == 1) { ?>
        <div class="row">
            <div class="col-xs-12 col-lg-4" style="padding: 15px 0;">
                <div class="form-group">
                    <div class="col-xs-10">
                        <v-select :options="branches" label="Brunch_name" v-model="selectedBranch" @input="onChangeBranch"></v-select>
                    </div>
                    <div class="col-xs-2">
                        <button type="button" @click="getGraphData">Submit</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            getBranch() {
                axios.get("/get_branches")
                    .then(res => {
                        ...
                        this.branches.unshift({brunch_id: "", Brunch_name:"All Branch"})
                    })
            },
            onChangeBranch() {
                this.branchId = this.selectedBranch.brunch_id
            },
            getGraphData() {
                let data = {
                    branchId: this.currentBranchId == 1 ? this.branchId == '' ? "" : this.branchId : this.currentBranchId
                }
                axios.post('/get_graph_data', data).then(res => {
                    this.salesData = [
                        ['Date', 'Sales']
                    ]
