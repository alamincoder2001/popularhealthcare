...
class Login extends CI_Controller {
    ...
    function procedure(){
		...
		$query = $this->db->query("SELECT u.User_SlNo, u.User_ID, u.FullName, u.User_Name, u.userBrunch_id, u.Employee_ID, u.UserType, u.image_name as user_image, u.status AS userstatus, br.brunch_id, br.Brunch_name, br.Brunch_sales, e.Employee_Name, e.Category_ID FROM tbl_user AS u LEFT JOIN tbl_brunch AS br ON br.brunch_id = u.userBrunch_id LEFT JOIN tbl_employee AS e ON e.Employee_SlNo = u.Employee_ID where br.status = 'a' and u.User_Name = ? AND u.User_Password = ?", [$user, $pass]);
		...
		if(isset($data)){
			...
			if ($data->userstatus=='a') {
				...
				$sdata['EmployeeID']    = $data->Employee_ID;
				...
				$sdata['Employee_name'] = $data->Employee_Name;
