...
class Employee extends CI_Controller
{
    ...
    public function employee_insert()
    {
        ...
        $category_id      = $this->input->post('em_Category', true);
        $Reportingboss_Id = $this->input->post('Reportingboss_Id', true);
        ...
        $data['bio_id']                    = $bio_id;
        $data['Reportingboss_Id']          = $Reportingboss_Id;
        ...
        $employee_last_id = $this->db->insert_id();
        //insert reporting boss
        if(!empty($Reportingboss_Id)){
            $this->db->query("INSERT INTO tbl_reportingboss (Reportingboss_Id, employee_id, branch_id) VALUES ('$Reportingboss_Id','$employee_last_id','$this->brunch')");
        }
    ...
    public function get_reporting_boss()
    {
    ...
    public function employee_Update()
    {
        ...
        $reporting_boss = $this->db->query("SELECT * FROM tbl_reportingboss WHERE Reportingboss_Id = ? AND employee_id = ? AND branch_id = ?",[$Reportingboss_Id, $id, $this->brunch])->num_rows();
        if($reporting_boss == 0){
            $this->db->query("INSERT INTO tbl_reportingboss (Reportingboss_Id, employee_id, branch_id) VALUES ('$Reportingboss_Id','$id','$this->brunch')");
        }else{
            $this->db->query("UPDATE tbl_reportingboss SET Reportingboss_Id='$Reportingboss_Id',employee_id='$id',branch_id='$this->brunch' WHERE Reportingboss_Id = ? AND employee_id = ? AND branch_id = ?", [$Reportingboss_Id, $id, $this->brunch]);
        }
