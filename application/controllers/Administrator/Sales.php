...
class Sales extends CI_Controller
{
        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                $saleDetails = array(
                    ...
                    'SaleDetails_Discount' => $cartProduct->discount ?? 0,
    ...
    function sales_totalqtyrecord()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        }
        $data['title'] = "Sales Quantity Record";
        $data['content'] = $this->load->view('Administrator/sales/totalqtyreport', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getTotalQty()
    {
        try {
            ...
            if (isset($data->reportingBossId) && $data->reportingBossId != '') {
                $res = $this->db->query("SELECT e.Employee_SlNo, e.Employee_Name
                                        FROM tbl_employee e
            ...
            if (isset($data->reportingBossId) && $data->reportingBossId != "") {
                foreach ($res as $value) {
                    $value->salesQty = $this->db->query("SELECT
                                p.Product_SlNo,
                                p.Product_Code,
                                p.Product_Name,
                                pc.ProductCategory_Name AS Category,
                                ...
                                AND sm.employee_id = '$value->Employee_SlNo'
                                AND sd.SaleDetails_BranchId = '$this->sbrunch' $clauses) AS product_qty,
                                
                                (SELECT IFNULL(SUM(sd.SaleDetails_TotalQuantity * sd.SaleDetails_Rate),0)
                                FROM tbl_saledetails sd
                                LEFT JOIN tbl_salesmaster sm ON sm.SaleMaster_SlNo = sd.SaleMaster_IDNo
                                LEFT JOIN tbl_employee em ON em.Employee_SlNo = sm.employee_id
                                WHERE sd.Status = 'a' 
                                AND sd.Product_IDNo = p.Product_SlNo
                                AND sm.employee_id = '$value->Employee_SlNo' 
                                AND sd.SaleDetails_BranchId = '$this->sbrunch' $clauses) AS product_price
                            FROM tbl_product p
                            LEFT JOIN tbl_productcategory pc ON pc.ProductCategory_SlNo = p.ProductCategory_ID
                            WHERE p.status = 'a'
