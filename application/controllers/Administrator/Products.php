...
class Products extends CI_Controller {
    ...
    public function getCurrentStock(){
        ...
        $stock = $this->mt->currentStock($clauses, $this->session->userdata('BRANCHid'));
        $res['stock'] = $stock;
        $res['totalValue'] = array_sum(
            array_map(function($product){
                return $product->stock_value;
            }, $stock));
        $res['saleTotalValue'] = array_sum(
            array_map(function($product){
                return $product->sale_stock_value;
            }, $stock));
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(prd.PurchaseReturnDetails_ReturnQuantity), 0) 
                    ...
                    where prd.PurchaseReturnDetailsProduct_SlNo = p.Product_SlNo and prd.Status = 'a'
