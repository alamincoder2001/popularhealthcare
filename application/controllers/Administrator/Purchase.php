...
class Purchase extends CI_Controller
{
    ...
    public function addPurchase()
    {
        ...
        try{
            ...
            foreach($data->cartProducts as $pro){
                $BatchCount = $this->db->query("select * from tbl_purchasedetails where Product_IDNo = ? and batch_no = ?",[ $pro->productId, $pro->batch_no ])->num_rows();
                
                if($BatchCount > 0) {
                    $res = ['success'=>false, 'message'=> 'Batch No already exists'];
                    echo json_encode($res);
                    exit;
                }
            }
